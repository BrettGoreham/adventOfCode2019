...
public class day3 {
    ...
    public static void main(String[] args) {
        ...
        for(String instruction : line1Instructions) {
            ...
            for (int i = 0; i < repetitions; i++) {
                ...
                Point point = new Point(xCord, yCord, ++stepsToNow);
                setOfFirstLinePoints.putIfAbsent(point, point);
            }
        }

        //Reset To Origin for Second Line start
        xCord = origin.x;
        yCord = origin.y;
        stepsToNow = 0;

        HashMap<Point, Point> crossingPoints = new HashMap<>();

        for(String instruction : getInstructions(line2)) {
            char direction = instruction.charAt(0);
            int repetitions = Integer.valueOf(instruction.substring(1));
            ...
            for (int i = 0; i < repetitions; i++) {
                ...
                Point pointInLine2 = new Point(xCord, yCord, ++stepsToNow);

                if (setOfFirstLinePoints.containsKey(pointInLine2)) {
                    Point crossingPoint = new Point(pointInLine2.x, pointInLine2.y, pointInLine2.steps + setOfFirstLinePoints.get(pointInLine2).steps);

                    crossingPoints.putIfAbsent(crossingPoint, crossingPoint);
                }
            }
        }

        Optional<Integer> manhattanDistance = crossingPoints.values().stream().map(point -> point.manhattanDistanceBetween(origin)).min(Comparator.comparingInt(o -> o));
        Optional<Integer> minSteps = crossingPoints.values().stream().map(point -> point.steps).min(Comparator.comparingInt(o -> o));

        System.out.println("Results:\n");
        if( manhattanDistance.isPresent()) {
            System.out.println("manhattanDistance : " + manhattanDistance.get());
        }
        if (minSteps.isPresent()) {
            System.out.println("Minimum Steps : " + minSteps.get());
        }
    }

    private static List<String> getInstructions(String instructions){
    ...
    private static class Point {
        ...
        public int manhattanDistanceBetween(Point otherPoint) {
        ...
        public String toString() {
            return "(" + x + "," + y + ")" + " steps To Here : " + steps ;
        }
    }
}
